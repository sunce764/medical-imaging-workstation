...
将仓库整理为可展示状态前，对全部文档做了一轮**保真校准**——原则是「文档里每个数字与声明都必须与代码实况一致，不夸大、不虚构」。逐项以实测值对齐：

| 校准项 | 原表述 | 改为（实测） |
|--------|--------|--------------|
| 代码量 | ~4,700 行 | ~4,100 行应用代码 / 13 模块 |
| 回归套件 | 80 项 / 80-check | 102 项 |
| 覆盖率 | ≈66% | ≈67%（`ai_engine` 87% / `main` 82%） |
| 目录结构 | 缺 3 个新模块 | 补 `quantify`/`segmentation`/`mpr_geometry`，按「UI 层 / 纯计算 / 资源」分组 |

此外：

- **打包如实收敛**：扁平模块布局下，wheel 不含 `style.qss`/模型等资源，`pip install` 得到的是缺资源的降级应用；故移除 `[project.scripts]` 这一过度承诺，明确 `pyproject.toml` 用于项目元数据/依赖/工具配置，应用以 `python main.py` 运行。
- **作者声明去 solo 化**：README「为本人独立编写」→「由本人设计、主导开发与验证」。代码为 AI 辅助开发，提交历史如实保留 `Co-Authored-By` 标注，不作「独立/solo 编写」声称。
- 新增 `LICENSE`（保留所有权利，与软著立场一致）+ README 中英双语导航；第三方组件署名逐一核对。

原则：**宁可少说，不可失真。** 本项目最强的资产是「可核验的诚实」，任何注水都会反噬它。
