...
#   xy 平面内**默认**不做配准；勾选「配准」后由 registration.py 做**平面内刚性配准**
#   （平移 + 旋转，带 NCC 安全阀，不升则不采用），**形变配准仍未实现**。未配准时差值反映的是
#   「同一解剖层面上的密度差异 + 未校正的体位/呼吸相位差异」之和，
#   只能作定性参考，不能当作临床意义上的病灶变化量。
#   真正的随访定量还需要 deformable registration——那一层本项目未实现，呼吸导致的器官
#   形变不会被校正；故即使勾选了配准，结论仍只宜作定性参考。
# =============================================================================
