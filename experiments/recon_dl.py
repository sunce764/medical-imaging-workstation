...
#
# 【与既有工作的关系 —— 必须先说清楚】
#   本文件的网络（U-Net 主干 + 跳连 + 残差形式：预测伪影再相减，用于 FBP 图像域
#   后处理）在架构与训练目标上等同于 FBPConvNet：
#     K. H. Jin, M. T. McCann, E. Froustey, M. Unser, "Deep Convolutional Neural
#     Network for Inverse Problems in Imaging", IEEE TIP 26(9):4509-4522, 2017.
#   本实现是**独立写成**的（不用 MONAI/nnU-Net，自己搭网络本就是本研究目的之一），
#   但它不是新架构。可核实的差异只有两点：该文最稀疏做到 50 视角，本研究做到 15；
#   该文的第三方复现报告约 22M 参数，本研究 1.9M。
#   **不主张性能超越它**——口径（SNR dB vs RMSE 比）、几何与数据都不可比。
#   另注：该文的对照基线包含 TV 正则迭代重建，且其结论限定为 CNN 在「更真实的
#   模体」上胜出；在合成椭圆模体上 TV 更强。本研究此前缺 TV 基线——现已由
#   recon_tv.py（ASD-POCS，recon.compute_asdpocs）补上，见 results/exp_c_asdpocs.csv。
#   注意那是**研究一的口径**（64×64、Poisson 光子噪声、oracle-stopped），与本脚本
#   的 15/60 视角、无噪投影、128×128 不可直接并列——两者不构成对同一问题的对比。
#
...
# 数据：随机模体家族，由代码生成、种子固定，**不使用任何患者数据**——这一部分别人重跑
#       得到逐字节相同的模体。**训练不在此列**：本文件的 torch 侧 RNG 是在已提交的
#       recon_dl_* 产物跑完之后才固定的（见 train_one），固定后未再重跑比对，因此
#       重训能否得到同一批数字属未验证。见 experiments/README.md 的 Reproducibility 一条。
#
# 用法：
