...
#
# 【训练标签用真值，不用教师输出】
#   更正：本注释此前写的理由是「用教师预测当标签，学生的 Dice 就变成衡量模仿教师
#   有多像」。**那是错的**，属于把训练信号与评估目标混为一谈。标准知识蒸馏
#   （Hinton, Vinyals & Dean, arXiv:1503.02531）是在教师软标签**加真值硬标签**的
#   加权目标上训练，而评估自始至终在真值测试集上——原文的 test error / WER 全部
#   是真值指标。同目录 seg3d_data.py 的表述一直是对的（「训练标签可以用教师预测，
#   但验证集绝不能用」），只有这里写岔了。
#
#   不用蒸馏的真正理由是数据泄漏：organs.onnx 是 TotalSegmentator v2 的权重，而
#   本研究的数据 TotalSegmentator-CT-Lite 派生自同一个上游数据集。**教师极可能在
#   本研究的 test 划分上训练过**（无法逐例证实，但数据集谱系使重叠几乎必然）。
#   在这个前提下拿教师预测当训练信号，等于把教师的测试集见闻注入学生，学生的
#   hold-out 就不再是真 hold-out。故本研究训练与验证**一律用数据集自带的真值标注**。
#
# 用法：
