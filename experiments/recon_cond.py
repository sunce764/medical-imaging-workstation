...
def measure(n=64, angle_range=180.0, n_projs=(30, 60, 90)):
    ...
    for npj in n_projs:
        ...
        # 数值秩：以 max(m,n)·eps·σ_max 为阈（NumPy matrix_rank 的默认口径，双精度）
        tol = max(m_rows, n_cols) * np.finfo(np.float64).eps * smax
        rank = int((sv > tol).sum())
        cond = smax / smin if smin > 0 else float("inf")

        # —— 与 compute_dmr 实际行为对齐 ——
        # compute_dmr 用 np.linalg.lstsq(A, p, rcond=None)，A 是 float32，
        # 故 LAPACK 的截断阈是 max(m,n)·eps_float32·σ_max，比双精度阈大 8 个数量级。
        # 小于该阈的奇异方向被直接丢弃（等价于截断 SVD 正则化），
        # 因此决定 DMR 噪声放大的不是朴素 cond，而是**保留下来的最小奇异值**：
        # 最坏方向上的噪声增益 = ||A⁺||₂ = 1/σ_k。
        rcond32 = max(m_rows, n_cols) * float(np.finfo(np.float32).eps)
        cut = rcond32 * smax
        k = int((sv > cut).sum())
        ...
        print(f"[cond] views={npj:3d}  A={m_rows}x{n_cols}  "
              ...
              f"保留 {k} 向  σ_k={sigma_k:.4g}  噪声增益 1/σ_k={1.0/sigma_k:.4g}  "
