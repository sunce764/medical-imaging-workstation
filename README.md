...
|---|---|---|
| **Study I — reconstruction dose–quality** | Error saturates beyond ≈180 views; the best FBP filter switches from smoothing at sparse angles to sharp Ram-Lak at dense angles; ART is the most robust tested solver under Poisson photon noise. A later pass took the SVD of the same system matrices and **refuted this study's own explanation** for the least-squares blow-up: the condition number is worst at the *sparsest* sampling, not near the square regime. What replaces it is deliberately qualitative — no quantitative attribution of the spike is claimed. | Analytic 2-D Shepp-Logan phantom; matrix methods are limited to ≈64×64; ART/SIRT iteration counts fixed, not tuned. [Preprint](docs/preprint_recon.md) · [conditioning CSV](experiments/results/exp_c_conditioning.csv) |
...
- On the author's machine the full suite is **586 checks** and `SKIP_REAL_DATA=1` reaches **495**. **A clean CI runner reaches fewer**, and the run log is the authority for that number, not this README: at commit `7a09744` CI logged **458** where the same command logged 471 locally. The gap is not flakiness — it is checks that need artefacts a clean clone does not have (the uncommitted `.onnx.data` blobs and `.pt` checkpoints), which simply do not execute there. What stays out of CI is everything requiring the RIDER series or those blobs — *not* the interaction layer, which is covered in CI through synthesised DICOM and synthesised mouse/wheel events.
...
```bash
python tests/test_gui.py                     # full suite: 586 locally; requires local RIDER data
SKIP_REAL_DATA=1 python tests/test_gui.py    # 495 locally; fewer on a clean CI runner (see above)
ruff check .                                 # lint
