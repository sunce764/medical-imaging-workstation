...
- **Two weight blobs are `artifact not distributed`**, and neither is recoverable from a plain `git clone`. Both are pinned by SHA-256 in [`models/CHECKSUMS.sha256`](models/CHECKSUMS.sha256) — the digests identify the exact bytes every model-derived number here came from, and [Architecture → Getting the weights](docs/ARCHITECTURE.md#getting-the-weights) states what about their origin was *not* recorded:
...
- The full suite is **562 checks**; CI runs the **471** that need no local research data. What stays out of CI is everything requiring the RIDER series or the 119 MB weights — *not* the interaction layer, which is covered in CI through synthesised DICOM and synthesised mouse/wheel events.
...
```bash
python tests/test_gui.py                     # full suite: 562 checks; requires local RIDER data
SKIP_REAL_DATA=1 python tests/test_gui.py    # 471 data-independent checks used by CI
ruff check .                                 # lint
