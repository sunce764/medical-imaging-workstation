...
**A defect in the evaluation, not in the model.** The same weights score **0.490 or 0.746** Dice — the only thing that changed is the size of the inference tensor, and not one input voxel. `InstanceNorm3d` normalises per sample over the spatial dims; after HU rescaling, air and zero-padding hold the same value, so enlarging the tensor erased **99.3 %** of the predicted foreground (225,374 → 1,529 voxels). Five targeted controls each eliminate a different competing explanation — targeted, not statistically independent: they share the same cases and weights, and each is aimed at one rival account.
...
**A pilot result that did not survive.** A 3-case pilot advertised **+0.205** for adding z-overlap; the full test split — 61 held-out cases, 59 of which carry at least one in-scope organ and are therefore evaluable — says **+0.0133** [+0.0072, +0.0194]. Both numbers are in this repository. The full-split run exists precisely to stop an outlier from becoming the headline.
...
- On the author's machine the full suite is **629 checks** and `SKIP_REAL_DATA=1` reaches **539**. **A fresh local clone reaches 520**; the last remote run at code baseline `0a9bfca` recorded 512. The gap is not flakiness: checks that need artefacts a clean clone does not have (the undistributed `.onnx.data` blobs and `.pt` checkpoints) simply do not execute there. This clean-clone method has also matched CI historically: at commit `7a09744` both gave **458** ([run log](https://github.com/sunce764/medical-imaging-workstation/actions/runs/32590281440)). The run log remains the authority for remote state. What stays out of CI is everything requiring the RIDER series or those blobs — *not* the interaction layer, which is covered through synthesised DICOM and synthesised mouse/wheel events. The custom runner also promotes uncaught Qt signal/slot exceptions to failures, so a printed traceback can no longer coexist with exit code 0.
...
```bash
python tests/test_gui.py                     # full suite: 629 locally; requires local RIDER data
SKIP_REAL_DATA=1 python tests/test_gui.py    # 539 locally; 520 in a fresh local clone (measured)
ruff check .                                 # lint
...
| [User manual](docs/manual_en.md) · [中文](docs/manual_zh.md) · [PDF](docs/manual_zh.pdf) | EN · 中文 | Screenshot-led guide to every user-facing feature. **Erratum, read before opening the PDF:** that file is the frozen V1.0 snapshot submitted for copyright registration and is deliberately not being edited while the filing is under examination, so its cover still calls the screenshot data "非患者数据 / not patient data". That is wrong — TotalSegmentator-CT-Lite is public, de-identified **human** CT, imaging real patients with no identifiable PHI. The two Markdown versions carry the corrected wording. |
...
- **The working tree has moved ahead of that snapshot**, and deliberately so — a filing is examined on the materials submitted, so those are frozen while the product keeps developing. Nothing here claims the current code *is* the submitted snapshot — nor that the filing has been registered; it has been submitted and is awaiting acceptance.
