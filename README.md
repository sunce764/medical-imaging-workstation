...
Most figures above are recomputable from the committed CSVs — but not all of them, so the exceptions are named rather than waved at: parameter counts come from the two committed `.onnx` graphs, the 207/29/61 split from `seg3d_data.split()` over the manifest, and the `8.44 / 9.09 GB` pair in Study IV was never written to a file at all (flagged at the point it is quoted).
...
- **Two weight blobs are `artifact not distributed`**, and neither is recoverable from a plain `git clone`:
  ...
  - `models/recon_dl_v20.onnx.data` (7.4 MB) — trained here, for Study III's learned reconstruction. Not committed because it is rebuildable, which is weaker than reproducible and is stated as such. Rebuilding takes **two** steps: `python experiments/recon_dl.py matrix` trains and writes the `.pt`, then `python experiments/recon_dl.py export` converts it into the ONNX external-data blob. **What comes back is a statistically equivalent model, not the same weights** — the phantom generation is seeded (`RandomState`), but `recon_dl.py` never calls `torch.manual_seed`, so weight initialisation and the `torch.randperm` shuffle are not pinned. Without it the reconstruction lab still runs; only the CNN post-processing view is unavailable.
