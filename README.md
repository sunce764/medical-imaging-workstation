...
Highlights: modular architecture (God-object decomposed into 5 UI mixins + 4 Qt-free compute modules that are unit-tested in isolation), a **155-check** offscreen-Qt regression suite with CI, defensive DICOM handling, and a reconstruction stack (Radon / BP / FBP / DFR / DMR / ART / SIRT) whose DFR, DMR, ART and SIRT inverse solvers are written from first principles on top of scikit-image's `radon`/`iradon` projector.
...
tests/test_gui.py  回归测试套件（155 项）　·　experiments/ 量化研究 + 复现脚本/图表/CSV
...
```bash
python tests/test_gui.py                     # 完整回归（155 项，需同目录 肺癌/ 真实数据）
...
- **CI**（`.github/workflows/ci.yml`）：每次 push/PR 在 Ubuntu 跑 `ruff` + 数据无关测试子集（离屏 Qt，无需真实数据或 119MB 权重）。CI 跑的是子集而非全套——交互层（重建实验室 / 双序列对比 / ROI 拖缩 / Cine）需本地真实数据才会执行，故「CI 全绿」不等于全部 155 项都跑过。
