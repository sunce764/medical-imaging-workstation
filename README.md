...
- **Engineered for review** — a God-object decomposed into 5 UI mixins + 7 Qt-free compute modules; a **226-check** offscreen-Qt regression suite with CI (reconstruction algorithms carry numerical-correctness assertions, not just "finite"); defensive DICOM handling.
...
```bash
python tests/test_gui.py                     # full suite: 226 checks (needs local RIDER data)
