...
- On the author's machine the full suite is **629 checks** and `SKIP_REAL_DATA=1` reaches **539**. **A fresh local clone reaches 520**. The most recent dated remote verification is the **2026-08-25 verified baseline `5c555ef`**: [run `32831264615`](https://github.com/sunce764/medical-imaging-workstation/actions/runs/32831264615) recorded **520 PASS / 0 FAIL**, **81% coverage**, and **Ruff PASS** with `event=workflow_dispatch`. The gap from the local counts is not flakiness: checks that need artefacts a clean clone does not have (the undistributed `.onnx.data` blobs and `.pt` checkpoints) simply do not execute there. This clean-clone method has also matched CI historically: at commit `7a09744` both gave **458** ([run log](https://github.com/sunce764/medical-imaging-workstation/actions/runs/32590281440)). Subsequent remote state is authoritative only when bound to a GitHub Actions run and its exact `headSha`. What stays out of CI is everything requiring the RIDER series or those blobs — *not* the interaction layer, which is covered through synthesised DICOM and synthesised mouse/wheel events. The custom runner also promotes uncaught Qt signal/slot exceptions to failures, so a printed traceback can no longer coexist with exit code 0.
