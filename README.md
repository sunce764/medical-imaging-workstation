...
- The full suite is **530 checks**; CI runs the **439** that need no local research data. What stays out of CI is everything requiring the RIDER series or the 119 MB weights — *not* the interaction layer, which is covered in CI through synthesised DICOM and synthesised mouse/wheel events.
...
```bash
python tests/test_gui.py                     # full suite: 530 checks; requires local RIDER data
SKIP_REAL_DATA=1 python tests/test_gui.py    # 439 data-independent checks used by CI
ruff check .                                 # lint
