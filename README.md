...
**Per-voxel confidence alongside every measurement.** Each organ row carries the model's softmax max-class probability and its 5th percentile — the low percentile is the revealing one, since errors concentrate at boundaries. Entries whose max-class probability falls below 0.9 are flagged. In the run below no organ's mean probability drops that far, but the **5th percentile** separates them clearly: the gallbladder (`conf 0.91 / p5 0.59`) has the lowest, i.e. its least-confident voxels are the least confident anywhere in the volume, matching what the spacing ablation independently found to be the most fragile structure.
...
```bash
python tests/test_gui.py                     # 2026-08-27 local run: 925 full-suite checks; local RIDER present
