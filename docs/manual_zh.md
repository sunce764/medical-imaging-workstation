...
> 本说明书随软件版本 V1.0 编写。文中截图均使用**公开数据集 TotalSegmentator-CT-Lite（CC-BY-4.0）**演示——**公开、已去标识的人体 CT**。这些影像来自真实患者：数据集去掉的是**可识别性**，不是临床来源，因此不含任何可识别的个人隐私信息（PHI），但称其「非患者数据」并不成立。患者信息栏已明确标注为公开数据。
