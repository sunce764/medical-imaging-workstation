...
**开发语言**：Python。
**软件规模**：应用代码约 4,100 行，划分为 13 个模块；配套 219 项自动化回归测试。
...
单击**「三维重建预览」**，软件对**当前画笔目标所选器官**做等值面提取（marching cubes），弹窗显示四个方位角的静态渲染，并给出**表面积、体积、球形度与面片数**；可进一步**导出 STL**（ASCII 格式，单位 mm，供 3D 打印或外部软件打开）。

渲染为纯计算实现（正交投影 + Lambert 着色 + 画家算法深度排序），不依赖 OpenGL 或 VTK，故无需 GPU；代价是没有透视、阴影与交互旋转，只作静态多角度预览。

> **精度说明**：以解析球体验算，**网格体积误差 <1%，可直接使用**；但**表面积系统性高估约 9%**（marching cubes 在体素化曲面上的阶梯效应），球形度因此略低于真值。故表面积与球形度**宜作同条件下的相对比较**（同一病例的不同器官、同一器官的随访变化），不宜当作绝对几何量引用。
...
> **重要限制**：本功能只做了 **z 轴层面配准**，**平面内未做任何刚性或形变配准**。因此所报差异中混有未校正的体位与呼吸相位差异，**只能作定性参考，不是临床意义上的病灶变化量**。真正的随访定量需要 rigid/deformable registration，本软件未实现。此外，既往序列不做 AI 推理，故**不提供器官级体积变化**。
