...
> **版本对应关系**：随软件著作权登记提交的说明书 **PDF 为 V1.0 快照**，V1.0 的内容以该 PDF 为准，本文档不对其作任何追溯修改。本 Markdown 源在 V1.0 之后继续维护，已记录若干 V1.0 尚未包含的功能（推理前的 spacing 重采样、逐体素置信度、模型说明卡、重建实验室内置模体等），故与该 PDF 存在差异。这是源码正常演进的结果，不构成对已登记版本的更正。
...
**开发语言**：Python。
**软件规模**：应用代码约 6,000 行，划分为 18 个模块；配套 487 项自动化回归测试（其中 396 项不依赖真实数据，随 CI 运行）。
...
> **spacing 重采样对形状特征的影响（实测）**：AI 推理会先把体积重采样到模型训练 spacing（1.5mm），蒙版边界随之按该网格量化。以解析球实测，这使表面积偏差由 +0.3% 变为 **+1.9%**、体积偏差由 −0.3% 变为 **−1.0%**、球形度由 0.995 降至 0.974。量级可控（Taubin 平滑吸收了大部分），但用形状特征做定量比较时应知道它包含这一项。
