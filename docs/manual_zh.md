...
**开发语言**：Python。
**软件规模**：应用代码分为 UI mixin 与无 Qt 计算模块；2026-08-27 的一次本机实测，全套回归为 917 PASS，`SKIP_REAL_DATA=1` 子集为 825 PASS。这些是本地结果，不是 fresh-clone、coverage 或 remote-CI evidence。
...
> **推理前的 spacing 重采样**：所用模型（nnU-Net v2）要求先把体积重采样到其训练体素间距（1.5mm 各向同性），软件会自动完成这一步，并在状态栏注明。跳过它的代价经实测：体素间距偏离一倍时平均 Dice 由 0.922 降至 0.799，且小器官最先失效。代价是蒙版边界按 1.5mm 网格量化，映射回更细的原始分辨率后呈阶梯状——**结构准确度提升，像素级边界精度下降**。若序列的体素间距本就接近 1.5mm，或扫描范围过大（重采样后会超出内存上限），软件会跳过该步骤。
