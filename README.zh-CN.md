...
- **单例的 Dice 确实偏乐观——已实测修正。** 研究二的 21 器官 Dice ≈ 0.92 出自**单一序列（*n = 1*）**；研究四在 **57 例**带真值的公开 CT 上重测了 5 个肺叶，结果为 **0.8867，95% CI [0.859, 0.914]**，而同样这五叶在单例上是 0.956–0.991。差距最大的是**右肺上叶：单例 0.967 → 57 例 0.727**（CI [0.590, 0.845]，仅 31 例在场）——单例恰好抽到了容易的那一例。研究二的 21 器官总体数值仍是 *n = 1*，尚未逐一重测。
