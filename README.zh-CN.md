...
| | 实测 | 为什么这一条才是关键 |
|---|---|---|
| **缺陷出在评估，不在模型** | 同一份权重得 **0.490 或 0.746** Dice——唯一变化的是**推理张量的尺寸**，输入体素一个都没动 | `InstanceNorm3d` 逐样本在空间维求统计量；HU 归一化后，空气与补零同为 0。放大张量抹掉了 **99.3%** 的预测前景（225,374 → 1,529 体素）。五条互不依赖的对照，各自排除一种竞争解释 |
| **一次改动了产品的消融** | Dice **0.684 → 0.840**，配对 **20 例全部改善**（Wilcoxon *p* = 1.9×10⁻⁶）；推理 **100s / 8.8GB → 37s / 3.0GB** | 引擎此前静默跳过了 nnU-Net 强制要求的、重采样到训练 spacing 这一步。先测量、后修复——而这次修复让它同时更准也更省 |
| **一个没能站住的试跑结果** | 3 例试跑曾给出 **+0.205**；全量 59 例只有 **+0.0133**［+0.0072, +0.0194］ | 两个数字都留在仓库里。跑全样本的意义，正在于拦住离群值成为头条 |
