...
**三维多器官 CT 分割与断层重建**，封装在一个临床式 DICOM 工作站里（PySide6/Qt6，CPU-only）。界面之下：迭代求解器（DMR / ART / SIRT）、直接傅里叶重建与解析 Shepp-Logan 模体均按第一性原理自行实现；**两个网络从零训练**——用于稀疏视角重建的 1.9M 残差 U-Net，与用于肺叶分割的 0.35M 3D U-Net；随软件发布的 25 类 ONNX 分割模型来路无任何文档，其身份由本项目识别确证，并在 297 例公开 CT 上验证。仓库另附**四项正式记录的量化研究、两项多例验证，以及一次促成产品改动的消融实验**，把产品代码转化为可测、可复现的证据。
...
| 产品 | 运行环境 | 证据 | 数据边界 |
|---|---|---|---|
| DICOM 阅片 + AI 分割 + 重建实验室 | Python 3.10 · PySide6/Qt6 · CPU-only | 研究 I–IV · 57 例肺叶与 20 例多器官验证 · 一次修好真实缺陷的 spacing 消融 | 合成模体与公开去标识研究 CT；仓库不提交 PHI |
...
| **研究 IV —— 压缩分割模型，以及它暴露出的评估缺陷** | 从零训练的 0.35M 参数 3D U-Net，对照随软件发布的 31.2M 教师。给它打分时暴露出问题出在**评估**而非模型：`InstanceNorm3d` 逐样本在空间维求统计量，而 HU 归一化后空气与补零同为 0——把推理张量放大，**内容一个体素都没变，预测前景却消失 99.3%**（225,374 → 1,529）。同一份权重，只因张量尺寸不同而得 **0.490 或 0.746**。把同样的怀疑用到产品推理路径上——配对测量、全部 24 器官、覆盖 **test 集 61 例中含至少一个在场器官的 59 例**——得到全器官 Dice **+0.0133**［+0.0072, +0.0194］，**59 例中 54 例改善**，代价 1.18× 耗时与 +0.65GB。 | 五条互不依赖的对照——阴性对照、剂量-反应、内容固定的补零实验、归一化层前向 hook、训练集对照——各自排除一种竞争解释。3 例试跑曾给出 **+0.205**，全样本只有 **+0.013**；全样本这一跑的意义正在于拦住离群值成为头条。在留出的 test 集上、以同一条推理路径相比，学生比教师低 **0.4500**［-0.4877, -0.4118］（234 个叶次）——0.35M 的模型在此并未逼近 31.2M。[`seg3d_infer_bias.py`](experiments/seg3d_infer_bias.py) |
